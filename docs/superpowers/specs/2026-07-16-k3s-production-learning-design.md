...
~~OrbStack VM + k3s + kube-prometheus-stack + MinIO 进集群。首批告警规则：Kafka consumer lag 持续增长、pod 重启、磁盘水位，经 Telegram 推送到手机。~~ ✓ 已完成（详见 `docs/superpowers/specs/2026-07-21-phase1-k3s-cluster-foundation-design.md`，实施提交见 `042f814`）
...
✓ 已完成（环境仓库 `~/big-data-platform-envs/` 已创建，ArgoCD + Velero 已安装，三场毕业演练通过）
