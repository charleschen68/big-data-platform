...
1. Flink checkpoint/savepoint 指向 MinIO（S3 协议），完整演练一次 savepoint→停止→重启→恢复。
2. `eth_trade_settlement.py` 增加幂等键（按信号 ID 去重），防止重复下单。
3. fastjson 1.2.83（已知 RCE 链）替换为 fastjson2。
4. 密码从 Javadoc、`MyParameter` 默认值中清除，统一走 `.env`/启动参数。
5. `EthBacktestDecisionFunction` 补单元测试（决策核心、纯逻辑）。
